...
describe('statTarget', () => {
  it('parses a plain integer', () => {
    expect(
      statTarget({ value: '100', suffix: '%', label: { pt: '', en: '', es: '' } }),
    ).toBe(100)
  })
  it('parses a value with a thousands separator', () => {
    expect(
      statTarget({ value: '2,154', suffix: '', label: { pt: '', en: '', es: '' } }),
    ).toBe(2154)
  })
  it('parses a leading-number value like "5+"', () => {
    expect(
      statTarget({ value: '5', suffix: '+', label: { pt: '', en: '', es: '' } }),
    ).toBe(5)
  })
  it('returns 0 for a non-numeric value', () => {
    expect(
      statTarget({ value: '∞', suffix: '', label: { pt: '', en: '', es: '' } }),
    ).toBe(0)
  })
})
